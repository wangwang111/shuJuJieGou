...
public class XuanZe { 
	...
	public static void SelectSort(int R[]) {
		for(int i=0;i<R.length-1;i++) {			//做第i趟排序
			int k=i;
			for(int j=i+1;j<R.length;j++) {			//找出最小元素
				if(R[j]<R[k]) {
					k=j;
				}
			}
	 ...
	 */
	//筛选算法
	private static void sift(int R[],int low,int high) {    //R[low..high]
	...
	public static void HeapSort(int[] R) {
		int i;
		int temp;
		for(i=R.length/2;i>=1;i--) {		//循环建立初始堆
			sift(R, i,R.length);
		}
		
		for(i=R.length;i>=2;i--) {			//进行n-1次循环，完成堆排序
			temp=R[1];
			R[1]=R[i];
			R[i]=temp;
